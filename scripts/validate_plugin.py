...
def main() -> int:
    ...
    # === no re-networking: the de-networked security posture must not regress ===
    # This fork is pure-markdown — no MCP manifest, no server/dashboard tree, no vendored JS
    # blobs. A re-added .mcp.json, server/, dashboard/, or skills/start/ would silently
    # reintroduce a process spawn / listening socket / supply-chain blob on session open.
    print("== no re-networking (security posture) ==")
    for rel in (".mcp.json", "server", "dashboard", "skills/start"):
        check(not (ROOT / rel).exists(),
              f"{rel} is absent (de-networked)",
              f"{rel} was re-added — this fork must stay pure-markdown (no MCP server / listener / vendored blob)")
