...
**Stack-agnostic by design — it imposes no DB philosophy and writes no rules file.** `data-model` **detects** the repo's existing DB + migration conventions (from the live `migrations/`, the schema, and CLAUDE.md / the architecture map) and **follows** them. On a greenfield repo with no signal, it **confirms each schema choice with the user** (Socratic) instead of defaulting to a house style. What it applies regardless of stack is migration **safety** (staging, reversibility, FK indexes, zero-downtime decomposition, no-PII) — never a stance on `updated_at` vs not, hard vs soft delete, UUID vs sequence, or whether `CHECK` constraints are allowed. The size matrix (→ [`../_shared/size-matrix.md`](../_shared/size-matrix.md)) governs how much you produce; the aggregate-roots dialogue uses [`../_shared/ask-style.md`](../_shared/ask-style.md).
...
`data-model` imposes **no** DB philosophy. For each topic below it **follows the repo's detected convention**; on a greenfield repo with no signal it **confirms the choice with the user** — it never legislates a house style.
...
2. **Detect conventions (read-only — never write a rules file).** Inspect the existing `migrations/` folder + the schema + CLAUDE.md / the architecture map (Explore or `ls`) to detect the repo's conventions and **follow them**: migration naming (sequential `000022_*.sql` vs timestamped vs empty) + any statement-per-file rule (e.g. golang-migrate's transaction-per-file), plus PK strategy, audit/delete patterns, constraint usage, and string/JSON norms. Record the migration naming as a **promote-time hint** in the audit report (e.g. «sequential, next ≈ `000023` — `implement` assigns the real number at promotion, since another feature may promote first»). On a **greenfield** repo with no signal, the schema choices are confirmed with the user (steps 4–6), not defaulted to a house style. **Do not pick a final number, do not write into the live `migrations/`, and do not impose a convention the repo doesn't use** — staging happens in step 9, promotion in `implement`; flag any divergence in the report.
