...
A `layer: ui` task (present only when `sad.md` frontmatter `target_surfaces` declares a UI surface — `web-frontend` / `mobile-app` / `desktop-app`) runs through the **same TDD cycle** as any other task; it just follows the **repo's UI test convention** — component / e2e-through-UI runners detected from the repo, **not** a backend assumption. On **web** that's `package.json` scripts (Playwright / Storybook / a visual-diff tool / etc.); on **mobile** it's the platform's UI/snapshot runner (iOS: XCUITest + snapshot tests; Android: Espresso + Paparazzi/Roborazzi + Firebase Test Lab) — detected via command-detection's mobile rows + wrapper-lane step, never hard-coded. No engine change beyond that detection: the runner is whatever the repo already configures.
