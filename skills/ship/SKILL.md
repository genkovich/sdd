...
1. **Final verification — does it actually work.** Re-run the detected gate (unit + integration where available + lint + vet). Then **run the feature for real** against its acceptance criteria — not just "tests pass": start the app / hit the endpoint / exercise the flow and observe the spec's outcomes (e.g. the default-on read returns defaults; an invalid value is rejected). Concretely: **spot-check at least 3 of the most critical §5 AC outcomes** (fewer only if the spec has fewer; scale the count with the feature's breadth), and for each name the AC id + the behaviour actually observed — «AC-03: posted the same apply twice → one discount row» — so the verification is checkable, not a vibe. If the app can't be run here (no runtime, no Docker), say so explicitly and record what was verified vs deferred — never claim verified-working when only tests compiled.
   - **Mobile surface** (`target_surfaces` includes `mobile-app`): "hit the endpoint" doesn't apply — verify by **booting a simulator/emulator and driving the critical ACs through the UI** (tie to command-detection's device probe). Some ACs — camera, push, biometrics, background, real-device sensors — **can only be checked on a physical device**; name that subset explicitly as *device-deferred* rather than letting the whole feature degrade to "compiled only". The deferral is a **named list of device-only ACs**, not a blanket escape hatch.
...
4. **Detect the forge + propose the PR command.** Inspect the remote: `github.com` → `gh pr create`; `gitlab.com`/self-hosted GitLab → `glab mr create`; otherwise print the branch + body for manual creation. **Propose** the command — do not run a push/PR to a shared remote without the user's go-ahead, and never merge to main.
   - **Mobile surface — a ship is more than a PR.** A `mobile-app` closing ritual is a **store submission** or a **package publish**, not just an opened PR. Detect which (does the repo build an installable app, or publish a library to a registry — see [`../_shared/surfaces.md`](../_shared/surfaces.md)) and record the release steps in the changelog/PR body (step 2/3), tool-agnostically — name the *steps*, detect Fastlane lanes / the repo's release workflow if present, never hard-code a tool:
     - **App flavor** — version + build-number bump, code signing, target track (internal / beta / store), phased rollout %, dSYM / ProGuard-mapping upload.
     - **Library/SDK flavor** — per-module semver bump, target registries (SPM / CocoaPods / Maven / package repo), snapshot-vs-release channel, and a **public-API breaking-change** callout.
...
- **Treating a mobile ship as "just a PR".** A `mobile-app` ship is a **store submission or a registry publish** (version/build bump, signing, target track/registry, rollout / breaking-change note) — omitting that is the mobile parallel of forgetting the migration note.
- **Letting mobile verification silently degrade to "compiled only".** Boot a simulator/emulator and drive the critical ACs; defer only the *named* device-only ACs (camera / push / biometrics / background), never the whole feature.
